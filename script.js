...
class RunningApp {
    ...
    handleEventJoin(button) {
        ...
        const isParticipating = event.participants.includes(this.currentUser.uid || this.currentUser.id);
        ...
        if (!isParticipating) {
            ...
            event.participants.push(this.currentUser.uid || this.currentUser.id);
            ...
            // 取消报名
            const index = event.participants.indexOf(this.currentUser.uid || this.currentUser.id);
            if (index > -1) {
    ...
    updateEventsDisplay() {
        ...
        eventsContainer.innerHTML = Object.values(this.events).map(event => `
            <div class="event-card" data-event-id="${event.id}">
                ...
                <button class="btn-secondary ${event.participants.includes(this.currentUser?.uid || this.currentUser?.id) ? 'btn-primary' : ''}">
                    ${event.participants.includes(this.currentUser?.uid || this.currentUser?.id) ? '已报名' : '我要报名'}
                </button>
            </div>
        `).join('');
        
        // 重新绑定事件
        document.querySelectorAll('.event-card .btn-secondary, .event-card .btn-primary').forEach(btn => {
            if (btn.textContent === '我要报名' || btn.textContent === '已报名') {
                btn.addEventListener('click', (e) => {
            ...
            return `
                <div class="run-card" data-run-id="${run.id}">
                    <div class="run-header">
                        ...
                        <div class="run-date">
                            ${new Date(run.timestamp?.toDate?.() || run.timestamp || run.date).toLocaleDateString('zh-CN')}
                        </div>
                    </div>
                    <div class="run-stats">
                        <div class="stat-item">
    ...
    showLeaderboard(type) {
        ...
        this.runs.forEach(run => {
            const runDate = new Date(run.timestamp?.toDate?.() || run.timestamp || run.date);
